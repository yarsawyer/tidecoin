...
Before starting this tutorial, start the bitcoin node on testnet.

```bash
./build/bin/tidecoind -testnet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in Tidecoin. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/tidecoin-cli -testnet createwallet "participant_${n}"
done
```

`bitcoin rpc` can also be substituted for `tidecoin-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Tidecoin single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, and `m/49'/1'/0'` for P2WPKH-nested-in-P2SH accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Tidecoin will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./build/bin/tidecoin-cli -testnet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/tidecoin-cli -testnet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./build/bin/tidecoin-cli help importdescriptors`.
...
```bash
./build/bin/tidecoin-cli -testnet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/tidecoin-cli  -testnet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/tidecoin-cli  -testnet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/tidecoin-cli -testnet loadwallet "participant_${n}"; done
```

### 1.4 Fund the wallet

The wallet can receive testnet coins by generating a new address and funding it from a testnet faucet or another testnet wallet.
...
```bash
receiving_address=$(./build/bin/tidecoin-cli -testnet -rpcwallet="multisig_wallet_01" getnewaddress)
```
...
```bash
./build/bin/tidecoin-cli -testnet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Tidecoin Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Tidecoin transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid Tidecoin address.
...
```bash
balance=$(./build/bin/tidecoin-cli -testnet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/tidecoin-cli -testnet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/tidecoin-cli -testnet decodepsbt $funded_psbt

./build/bin/tidecoin-cli -testnet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/tidecoin-cli -testnet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/tidecoin-cli -testnet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/tidecoin-cli -testnet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/tidecoin-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/tidecoin-cli -testnet finalizepsbt $psbt_2 | jq -r '.hex')
