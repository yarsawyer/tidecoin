...
The Tidecoin daemon can be configured to act as a trusted "border
router", implementing the Tidecoin P2P wire protocol and relay, making
...
The ZeroMQ feature in Tidecoin requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ tidecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=unix:/tmp/tidecoind.tx.raw \
               -zmqpubhashtxhwm=10000

`tidecoin node` or `tidecoin gui` can also be substituted for `tidecoind`.
...
From the perspective of tidecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tidecoind directly. Furthermore, no information is
...
using. `tidecoind` appends an up-counting sequence number to each
