# User-space, Statically Defined Tracing (USDT) for Tidecoin

Tidecoin includes statically defined tracepoints to allow for more
...
**Note**: Tidecoin uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Tidecoin
...
Multiple tools can list the available tracepoints in a `tidecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Tidecoin, use `info probes` in `gdb`:

```
$ gdb ./build/bin/tidecoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/tidecoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/tidecoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/tidecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Tidecoin.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/tidecoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/bin/tidecoind -v
