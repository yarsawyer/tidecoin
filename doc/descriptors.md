...
Bitcoin Core v24 extended `wsh()` output descriptor with [Miniscript](https://bitcoin.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25.
...
- [Miniscript](https://bitcoin.sipa.be/miniscript/) expressions in `wsh` (P2WSH) functions.
...
- `pkh(KEY)`: P2PKH output for the given public key (use `addr` if you only know the pubkey hash).
...
- `multi(k,KEY_1,KEY_2,...,KEY_n)`: k-of-n multisig script using OP_CHECKMULTISIG.
- `sortedmulti(k,KEY_1,KEY_2,...,KEY_n)`: k-of-n multisig script with keys sorted lexicographically in the resulting script.
