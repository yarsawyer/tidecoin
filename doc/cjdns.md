# CJDNS support in Tidecoin Core

It is possible to run Tidecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Tidecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Tidecoin network and individual nodes.
...
before it will work with your Tidecoin Core node. This is described in steps
...
## Run Tidecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Tidecoin Core
...
When enabled, this option tells Tidecoin Core that it is running in an
...
IPv6 local network. This helps Tidecoin Core perform better address management:
...
CJDNS support was added to Tidecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `tidecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/tidecoin/tidecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Tidecoin Core

There are several ways to see your CJDNS address in Tidecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `tidecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `tidecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `tidecoin-cli help getnodeaddresses` for details.
