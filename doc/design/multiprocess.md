...
4. **Handling in tidecoin-node**
   ...
   - The `getBlockHash` method of the generated `Chain` server subclass in `tidecoin-wallet` receives a Cap’n Proto request object with the `height` parameter, and calls the `getBlockHash` method on its local `Chain` object with the provided `height`.
