...
The data directory is the default location where the Tidecoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.tidecoin/`
macOS    | `$HOME/Library/Application Support/Tidecoin/`
Windows  | `%LOCALAPPDATA%\Tidecoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `tidecoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Tidecoin blocks (dumped in network format, 128 MiB per file)
...
`./`               | `tidecoin.conf`       | User-defined [configuration settings](tidecoin-conf.md) for `tidecoind` or `tidecoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `tidecoind.pid`        | Stores the process ID (PID) of `tidecoind` or `tidecoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `tidecoind` or `tidecoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [tidecoin.conf](tidecoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`tidecoin-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).
...
| tidecoin.conf                                              | [Generated](../contrib/devtools/gen-tidecoin-conf.sh) configuration file    |
...
- README and tidecoin.conf files are included in binary releases but not installed in source builds.
...
There have been multiple reports of database corruption and data loss when using this filesystem with Tidecoin Core.
