...
Below are some notes on how to build Tidecoin for Windows.

The options known to work for building Tidecoin on Windows are:
...
Note that for WSL the Tidecoin source path MUST be somewhere in the default mount file system, for
example /usr/src/tidecoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\tidecoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/tidecoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/tidecoin --strip
```
