...
class NULLDUMMYTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        legacy_coinbase_addr = key_to_p2pkh(self.pubkey, chain=self.chain)

        self.coinbase_blocks = self.generatetoaddress(self.nodes[0], 2, legacy_coinbase_addr)  # block height = 2
        coinbase_txid = []
        for i in self.coinbase_blocks:
            coinbase_txid.append(self.nodes[0].getblock(i)['tx'][0])
        self.generatetoaddress(self.nodes[0], COINBASE_MATURITY, legacy_coinbase_addr)  # block height = COINBASE_MATURITY + 2
        self.lastblockhash = self.nodes[0].getbestblockhash()
        self.lastblockheight = COINBASE_MATURITY + 2
        self.lastblocktime = int(time.time()) + self.lastblockheight
        coinbase_value = Decimal(str(self.nodes[0].gettxout(coinbase_txid[0], 0)["value"]))
        amount_1 = coinbase_value - Decimal("1")
        amount_2 = amount_1 - Decimal("1")
        amount_3 = amount_2 - Decimal("1")
        amount_4 = amount_3 - Decimal("1")
        ...
        test2tx = self.create_transaction(txid=txid2, input_details=ms_unlock_details,
                                          addr=self.ms_address, amount=amount_3,
                                          privkey=self.privkey)
        ...
        test4tx = self.create_transaction(txid=test2tx.txid_hex, input_details=ms_unlock_details,
                                          addr=getnewdestination()[2], amount=amount_4,
                                          privkey=self.privkey)
        ...
        test5tx = self.create_transaction(txid=txid3, input_details={"scriptPubKey": test1txs[2].vout[0].scriptPubKey.hex(),
                                          "amount": amount_1, "witnessScript": wms["redeemScript"]},
                                          addr=getnewdestination(address_type='p2sh-segwit')[2], amount=amount_2,
                                          privkey=self.privkey)
