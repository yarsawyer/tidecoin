...
class SignRawTransactionWithKeyTest(BitcoinTestFramework):
    ...
    def successful_signing_test(self):
        ...
        rawTx = self.nodes[0].createrawtransaction(self.inputs, self.outputs)
        rawTxSigned = self.nodes[0].signrawtransactionwithkey(rawTx, self.signing_privkeys, self.inputs)
    ...
    def witness_script_test(self):
        ...
        self.send_to_address(p2sh_p2wsh_address["address"], Decimal("1.0"))
        ...
        spending_tx = self.nodes[0].createrawtransaction([unspent_output], {getnewdestination()[2]: Decimal("0.999")})
    ...
    def verify_txn_with_witness_script(self, tx_type):
        ...
        # Fund that address
        [txid, vout] = self.send_to_address(addr, Decimal("1.0"))
        # Now create and sign a transaction spending that output on node[0], which doesn't know the scripts or keys
        spending_tx = self.nodes[0].createrawtransaction([{'txid': txid, 'vout': vout}], {getnewdestination()[2]: Decimal("0.999")})
        spending_tx_signed = self.nodes[0].signrawtransactionwithkey(spending_tx, [embedded_privkey], [{'txid': txid, 'vout': vout, 'scriptPubKey': script_pub_key, 'redeemScript': redeem_script, 'witnessScript': witness_script, 'amount': Decimal("1.0")}])
        self.assert_signing_completed_successfully(spending_tx_signed)
    ...
    def prepare_offline_signing_vectors(self):
        # Build chain-native offline signing vectors (fake prevouts + matching keys).
        priv1, pub1 = generate_keypair(wif=True)
        priv2, pub2 = generate_keypair(wif=True)
        self.signing_privkeys = [priv1, priv2]
        self.inputs = [
            {"txid": "11" * 32, "vout": 0, "scriptPubKey": key_to_p2pkh_script(pub1).hex()},
            {"txid": "22" * 32, "vout": 0, "scriptPubKey": key_to_p2pkh_script(pub2).hex()},
        ]
        self.outputs = {getnewdestination()[2]: Decimal("0.1")}
