...
be used in Tidecoin Core with the `-asmap` option.
...
to binary before being used with Tidecoin Core.
...
the `tidecoin-cli getnodeaddresses` command.
...
```
tidecoin-cli getnodeaddresses 0 > addrs.json
```
