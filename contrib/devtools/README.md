...
Provides utilities for managing copyright headers of `The Tidecoin Core
...
---------------------------------------------------------
Updates all the copyright headers of `The Tidecoin Core developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The Tidecoin Core developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The Tidecoin Core developers
```
...
those cases:
```
// Copyright (c) <year> The Tidecoin Core developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The Tidecoin Core developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The Tidecoin Core developers` at the top of the
...
If the file already has a copyright for `The Tidecoin Core developers`, the
script will exit.
...
gen-tidecoin-conf.sh
====================

Generates a tidecoin.conf file in `share/examples/` by parsing the output from `tidecoind --help`. This script is run during the
release process to include a tidecoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-tidecoin-conf.sh
```
