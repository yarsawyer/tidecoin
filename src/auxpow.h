 ...
 * coinbase tx) and a second merkle branch to link the actual aux block
...
public:
    ...
    SERIALIZE_METHODS(CAuxPow, obj)
    {
        ...
        READWRITE(TX_WITH_WITNESS(obj.coinbaseTx), hashBlock, obj.vMerkleBranch, nIndex, obj.vChainMerkleBranch,
                  obj.nChainIndex, obj.parentBlock);
    }
    ...
    {
        return parentBlock.GetScryptPoWHash();
    }
     ...
     * on that!
     */
    static std::unique_ptr<CAuxPow> createAuxPow(const CBlockHeader& header);
     ...
     * the auxpow object for convenience.
     */
