...
static void mineBlock(const node::NodeContext& node, std::chrono::seconds block_time)
{
    ...
    const int height = WITH_LOCK(::cs_main, return node.chainman->ActiveChain().Height() + 1);
    while (!CheckProofOfWork(block, node.chainman->GetConsensus(), height)) ++block.nNonce;
