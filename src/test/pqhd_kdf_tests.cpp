...
BOOST_AUTO_TEST_CASE(vectors_leaf_falcon512_receive_0)
{
    ...
    const std::array<uint32_t, 6> path{
        HARDENED | 10007U, // purpose'
        HARDENED | 6868U,  // coin_type'
    ...
    const auto stream_key = pqhd::DeriveKeygenStreamKey(std::span<const uint8_t, 32>(leaf.node_secret), /*scheme_id=*/7,
                                                        std::span<const uint32_t>(path));
    BOOST_CHECK_EQUAL(
        HexStr(std::span<const uint8_t, 64>(stream_key)),
    ...
    const auto block0 = pqhd::DeriveKeygenStreamBlock(std::span<const uint8_t, 64>(stream_key), /*ctr=*/0);
    BOOST_CHECK_EQUAL(
    ...
    const auto block1 = pqhd::DeriveKeygenStreamBlock(std::span<const uint8_t, 64>(stream_key), /*ctr=*/1);
    BOOST_CHECK_EQUAL(
...
BOOST_AUTO_TEST_CASE(vectors_leaf_mldsa65_change_5)
{
    ...
    const auto stream_key = pqhd::DeriveKeygenStreamKey(std::span<const uint8_t, 32>(leaf.node_secret), /*scheme_id=*/10,
                                                        std::span<const uint32_t>(path));
    BOOST_CHECK_EQUAL(
        HexStr(std::span<const uint8_t, 64>(stream_key)),
