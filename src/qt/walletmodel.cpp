...
void WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) // Message from normal tidecoin:URI (tidecoin:123...?message=example)
...
bool WalletModel::bumpFee(Txid hash, Txid& new_hash)
{
    ...
    if (retval == QMessageBox::Save) {
        ...
        bool complete = false;
        const auto err{wallet().fillPSBT(std::nullopt, /*sign=*/false, nullptr, psbtx,
                                         complete, /*include_pqhd_origins=*/false)};
        if (err || complete) {
