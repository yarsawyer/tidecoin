...
void PSBTOperationsDialog::openWithPSBT(PartiallySignedTransaction psbtx)
{
    ...
    if (m_wallet_model) {
        size_t n_could_sign;
        const auto err{m_wallet_model->wallet().fillPSBT(std::nullopt, /*sign=*/false, /*bip32derivs=*/false, &n_could_sign, m_transaction_data, complete)};
        if (err) {
...
void PSBTOperationsDialog::signTransaction()
{
    ...
    const auto err{m_wallet_model->wallet().fillPSBT(std::nullopt, /*sign=*/true, /*bip32derivs=*/false, &n_signed, m_transaction_data, complete)};
