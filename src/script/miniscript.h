...
/** Miniscript is limited to P2WSH in this codebase. */
constexpr bool IsTapscript(MiniscriptContext)
{
    return false;
}
...
constexpr uint32_t MaxScriptSize(MiniscriptContext)
{
 ...
 * - exec+1 is the maximal stack size reached during execution.
 *
...
struct Node {
    ...
    //! The Script context for this node (P2WSH only).
