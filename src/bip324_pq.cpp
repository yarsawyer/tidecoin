...
void BIP324PQCipher::InitializeFromSharedSecret(std::span<const std::byte> shared_secret,
                                                bool initiator,
                                                bool self_decrypt) noexcept
{
    assert(shared_secret.size() == 32);

    // Determine salt (fixed string + network magic bytes)
    const auto& message_header = Params().MessageStart();
    std::string salt = std::string{"tidecoin_v2_kem"} +
        std::string(std::begin(message_header), std::end(message_header));

    std::array<std::byte, 32> secret;
    std::copy(shared_secret.begin(), shared_secret.end(), secret.begin());

    // Derive encryption keys from shared secret, and initialize stream ciphers and AEADs.
    const bool side = (initiator != self_decrypt);
    CHKDF_HMAC_SHA256_L32 hkdf(UCharCast(secret.data()), secret.size(), salt);
...
void BIP324PQCipher::Encrypt(std::span<const std::byte> contents,
...
uint32_t BIP324PQCipher::DecryptLength(std::span<const std::byte> input) noexcept
{
...
bool BIP324PQCipher::Decrypt(std::span<const std::byte> input,
                             std::span<const std::byte> aad,
                             bool& ignore,
